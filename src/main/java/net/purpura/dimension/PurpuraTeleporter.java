...
public class PurpuraTeleporter implements net.minecraftforge.common.util.ITeleporter{

    protected final ServerWorld level;

    public PurpuraTeleporter(ServerWorld p_i1963_1_) {
        this.level = p_i1963_1_;
    }

    /*
    public PortalInfo findDimensionEntryPoint(ServerWorld p_241829_1_, Entity playerEntity) {
        BlockPos blockpos = p_241829_1_.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, p_241829_1_.getSharedSpawnPos());

        return new PortalInfo(new Vector3d((double)blockpos.getX() + 0.5D, (double)blockpos.getY(), (double)blockpos.getZ() + 0.5D), playerEntity.getDeltaMovement(), playerEntity.yRot, playerEntity.xRot);
    }

    @Nullable
    @Override
    public PortalInfo getPortalInfo(Entity entity, ServerWorld destWorld, Function<ServerWorld, PortalInfo> defaultPortalInfo) {
        return findDimensionEntryPoint(destWorld,entity);
    }
     */
    ...
    public Optional<TeleportationRepositioner.Result> findPortalAround(BlockPos p_242957_1_, boolean p_242957_2_) {
        ...
        Optional<PointOfInterest> optional = pointofinterestmanager.getInSquare((p_242952_0_) -> {
            return p_242952_0_ == Purpura.POI_PURPURA_PORTAL.get();
            ...
            for(int l1 = -1; l1 < 2; ++l1) {
                for(int k2 = 0; k2 < 2; ++k2) {
                    for(int i3 = -1; i3 < 3; ++i3) {
                        BlockState blockstate1 = i3 < 0 ? Purpura.PURPURIUM_BLOCK.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
        ...
        for(int k1 = -1; k1 < 3; ++k1) {
            for(int i2 = -1; i2 < 4; ++i2) {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
                    ...
                    this.level.setBlock(blockpos$mutable, Purpura.PURPURIUM_BLOCK.get().defaultBlockState(), 3);
                }
            }
        }

        BlockState blockstate = Purpura.PURPURA_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_242956_2_);
